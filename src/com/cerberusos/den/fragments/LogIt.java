...
import com.cerberusos.den.utils.SuShell;
...
public class LogIt extends BaseSettingsFragment implements Preference.OnPreferenceChangeListener {
    ...
    private static final String PREF_CERBERUSOS_LOG_IT = "cerberusos_log_it_now";
    private static final String PREF_SHARE_TYPE = "cerberusos_log_share_type";

    private static final String LOGCAT_FILE = new File(Environment
        .getExternalStorageDirectory(), "cerberusos_logcat.txt").getAbsolutePath();
    private static final String LOGCAT_LAST_FILE = new File(Environment
        .getExternalStorageDirectory(), "cerberusos_logcat_last.txt").getAbsolutePath();
    private static final String LOGCAT_RADIO_FILE = new File(Environment
        .getExternalStorageDirectory(), "cerberusos_radiolog.txt").getAbsolutePath();
    private static final String KMSG_FILE = new File(Environment
        .getExternalStorageDirectory(), "cerberusos_kmsg.txt").getAbsolutePath();
    private static final String DMESG_FILE = new File(Environment
        .getExternalStorageDirectory(), "cerberusos_dmesg.txt").getAbsolutePath();

    private static final String HASTE_LOGCAT_KEY = new File(Environment
            .getExternalStorageDirectory(), "cerberusos_haste_logcat_key").getAbsolutePath();
    private static final String HASTE_LOGCAT_LAST_KEY = new File(Environment
            .getExternalStorageDirectory(), "cerberusos_haste_logcat_last_key").getAbsolutePath();
    private static final String HASTE_LOGCAT_RADIO_KEY = new File(Environment
            .getExternalStorageDirectory(), "cerberusos_haste_logcat_radio_key").getAbsolutePath();
    private static final String HASTE_KMSG_KEY = new File(Environment
            .getExternalStorageDirectory(), "cerberusos_haste_kmsg_key").getAbsolutePath();
    private static final String HASTE_DMESG_KEY = new File(Environment
            .getExternalStorageDirectory(), "cerberusos_haste_dmesg_key").getAbsolutePath();

    private static final String CERBERUSOS_HASTE = "http://haste.baikalos-rom.com/documents";
    private static final File sdCardDirectory = Environment.getExternalStorageDirectory();
    private static final File logcatFile = new File(sdCardDirectory, "cerberusos_logcat.txt");
    private static final File logcatHasteKey = new File(sdCardDirectory, "cerberusos_haste_logcat_key");
    private static final File logcatLastFile = new File(sdCardDirectory, "cerberusos_logcat_last.txt");
    private static final File logcatLastHasteKey = new File(sdCardDirectory, "cerberusos_haste_logcat_last_key");
    private static final File logcatRadioFile = new File(sdCardDirectory, "cerberusos_radiolog.txt");
    private static final File logcatRadioHasteKey = new File(sdCardDirectory, "cerberusos_haste_logcat_radio_key");
    private static final File dmesgFile = new File(sdCardDirectory, "cerberusos_dmesg.txt");
    private static final File dmesgHasteKey = new File(sdCardDirectory, "cerberusos_haste_dmesg_key");
    private static final File kmsgFile = new File(sdCardDirectory, "cerberusos_kmsg.txt");
    private static final File kmsgHasteKey = new File(sdCardDirectory, "cerberusos_haste_kmsg_key");
    private static final File shareZipFile = new File(sdCardDirectory, "cerberusos_logs.zip");
    ...
    private Preference mCerberusOSLogIt;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mCerberusOSLogIt = findPreference(PREF_CERBERUSOS_LOG_IT);
    ...
    protected void updateEnabledState(CheckBoxPreference changedPref, boolean newValue) {
        ...
        mCerberusOSLogIt.setEnabled(enabled);
    }

    @Override
    public boolean onPreferenceTreeClick(Preference preference) {
        if (preference == mCerberusOSLogIt) {
    ...
    public void makeLogcat() throws SuShell.SuDeniedException, IOException {
        ...
        if (shareHaste) {
            command += " | tail -c " + HASTE_MAX_LOG_SIZE + " > " + LOGCAT_FILE
                    + "&& curl -s -X POST -T " + LOGCAT_FILE + " " + CERBERUSOS_HASTE
    ...
    public void makeLogcatLast() throws SuShell.SuDeniedException, IOException {
        ...
        if (shareHaste) {
            command += " | tail -c " + HASTE_MAX_LOG_SIZE + " > " + LOGCAT_LAST_FILE
                    + "&& curl -s -X POST -T " + LOGCAT_LAST_FILE + " " + CERBERUSOS_HASTE
                    + " | cut -d'\"' -f4 | echo \"http://haste.baikalos-rom.com/$(cat -)\" > "
    ...
    public void makeLogcatRadio() throws SuShell.SuDeniedException, IOException {
        ...
        if (shareHaste) {
            command += " | tail -c " + HASTE_MAX_LOG_SIZE + " > " + LOGCAT_RADIO_FILE
                    + "&& curl -s -X POST -T " + LOGCAT_RADIO_FILE + " " + CERBERUSOS_HASTE
                    + " | cut -d'\"' -f4 | echo \"http://haste.baikalos-rom.com/$(cat -)\" > "
    ...
    public void makeKmsg() throws SuShell.SuDeniedException, IOException {
        ...
        if (shareHaste) {
            command += " | tail -c " + HASTE_MAX_LOG_SIZE + " > " + KMSG_FILE
                    + " && curl -s -X POST -T " + KMSG_FILE + " " + CERBERUSOS_HASTE
                    + " | cut -d'\"' -f4 | echo \"http://haste.baikalos-rom.com/$(cat -)\" > "
                            + HASTE_KMSG_KEY;
        } else {
            command += " > " + KMSG_FILE;
    ...
    public void makeDmesg() throws SuShell.SuDeniedException, IOException {
        ...
        if (shareHaste) {
            command += " | tail -c " + HASTE_MAX_LOG_SIZE + " > " + DMESG_FILE
                    + "&& curl -s -X POST -T " + DMESG_FILE + " " + CERBERUSOS_HASTE
                    + " | cut -d'\"' -f4 | echo \"http://haste.baikalos-rom.com/$(cat -)\" > "
                            + HASTE_DMESG_KEY;
        } else {
            command += " > " + DMESG_FILE;
    ...
    public void resetValues() {
        ...
        mCerberusOSLogIt.setEnabled(false);
